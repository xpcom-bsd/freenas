...
class NetworkConfigurationService(ConfigService):
    ...
    async def validate_general_settings(self, data, schema):
        ...
        for key in [key for key in data.keys() if 'nameserver' in key]:
            nameserver_value = data.get(data[key])
            if nameserver_value:
                try:
    ...
    async def do_update(self, data):
        ...
        if not (
            ...
            self.middleware.call('notifier.failover_licensed')
        ):
            for key in ['gc_hostname_virtual', 'gc_hostname_b']:
                data.pop(key, None)

        new_config.update(data)
        verrors = self.validate_general_settings(data, 'global_configuration_update')  # TODO set schema name for validation
        if verrors:
            raise verrors
        ...
        if (
            ...
            if (
                'gc_hostname_virtual' in new_config.keys() and
                new_config['gc_hostname_virtual'] != config['gc_hostname_virtual']
            ):
                srv_service_obj = self.middleware.call(
                    'datastore.query',
                    'service.service',
                    [('srv_service', '=', 'nfs')]
                )
                nfs_object = self.middleware.call(
                    'datastore.query',
                    'services.nfs',
                )
                if len(srv_service_obj) > 0 and len(nfs_object) > 0:
                    ...
                    if (
                        ...
                        self.middleware.call('service.restart', 'ix-nfsd', {'onetime': False})
                        services_to_reload.append('mountd')

            for service_to_reload in services_to_reload:
                self.middleware.call('service.restart', service_to_reload, {'onetime': False})
